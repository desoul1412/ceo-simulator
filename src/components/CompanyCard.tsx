...
export function CompanyCard({ company, isSelected, onSelect }: CompanyCardProps) {
  ...
  return (
    ...
    >
      {/* Company name */}
      <div style={{ fontSize: 13, color: '#e0eaf4', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>
        {company.name}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 9, color: sColor, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
          {company.status}
        </span>
      </div>

      {/* Stats row */}
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#4a6080' }}>
      ...
      {company.ceoGoal && (
        <div style={{
          ...
          fontSize: 9, color: '#00ffff', lineHeight: 1.3,
